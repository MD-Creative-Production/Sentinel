        ...
        <tbody className="divide-y divide-gray-200 dark:divide-gray-800">
          {entries.map(entry => (
            ...
            >
              ...
              <td className="px-6 py-4">
                <span
                  className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${networkColors[entry.network] ?? 'bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-300'}`}
                >
                  {entry.network}
                </span>
              </td>
              ...
              <td className="px-6 py-4">
                <div className="flex gap-1 flex-wrap">
                  {entry.tags.map(tag => (
                    <span
                      key={tag}
                      className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400"
                    >
                      {tag}
                    </span>
                  ))}
              ...
              <td className="px-6 py-4">
                ...
                >
                  <span
                    className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${entry.enabled ? 'translate-x-4' : 'translate-x-0'}`}
                  />
                </button>
              </td>
